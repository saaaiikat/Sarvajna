...
import { type ChatStreamEvent } from "@sarvajna/shared";
import { isSuppoertedChatModel, resolveChatModel } from "../lib/models";

const submitSchema = z.object({
  content: z.string(),
  mode: z.enum(Mode),
  model: z.string().refine(isSuppoertedChatModel, "Unsupported model"),
});
...
async function streamAIResponse(
  ...
  const resolvedModel = resolveChatModel(model as any);
  let fullText = "";
...
const app = new Hono()
  .post("/:sessionId/resume", async (c) => {
    ...
    if (!isSuppoertedChatModel(resumableMessage.model)) {
      return c.json({ 
